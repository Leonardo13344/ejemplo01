...
int main(int argc, char** argv){
   ...
   //int parte_por_rank = (data.size() / 2 ) / 4;
    ...
    if(rank == 0)
    {
        ...
        MPI_Isend(&pares[5], 15, MPI_INT, 1, 0, MPI_COMM_WORLD, &request);
        MPI_Isend(&pares[20], 15, MPI_INT, 2, 0, MPI_COMM_WORLD, &request);
        MPI_Isend(&pares[35], 15, MPI_INT, 3, 0, MPI_COMM_WORLD, &request);
