...
int main(int argc, char** argv){
    int rank;
    int size;
    std::vector<int> data {5,6,4,8,10,22,54,23,1};
    ...
    if(rank == 0)
    {
        ...
        MPI_Send(&tamanioList2, 1, MPI_INT, 1, 0, MPI_COMM_WORLD);
        ...
        int sizeSortedList;
        MPI_Recv(&sizeSortedList, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        //elementos ordenados
        for(int i= 0; i < sizeSortedList; i++){
            int element;
            MPI_Recv(&element, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    else if( rank == 1)
    {
        ...
        {
            int elemento;
            MPI_Recv(&elemento, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            listaRecibida.push_back(elemento);
        }

        std::list<int> sortedList = bubbleSort(listaRecibida);
        int tamanioListSorted = sortedList.size();
        MPI_Send(&tamanioListSorted, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
